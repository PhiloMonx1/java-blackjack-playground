...
3. `Player` 객체 : 플레이어, 딜러로 나뉨<br>
   - 공통점
   1. 카드 리스트를 가진다.
   2. 카드를 뽑을 수 있다. (조건은 다름)
      - Player : input 값이 true 일 때
      - Dealer : isStay 값이 false 일 때
   3. 카드 합산 값이 21을 초과할 경우 Bust다
   4. 블랙잭이 가능하다.
   - 차이점 (Player)
   1. 이름을 가진다.
   2. 배팅금을 가진다.
   3. 카드를 뽑을지 말지 결정할 수 있다.
   - 차이점 (Dealer)
   1. 첫번째로 받은 카드는 공개되지 않는다.
   2. 카드 합산 값이 16이하라면 카드를 한 장 더 뽑고 17이상이라면 뽑지 않는다.

   > 블랙잭에서 플레이어가 할 수 있는 액션이 무엇이 있을까?
   - 배팅하기
   - 카드 한장을 더 받을지 결정하기 (Hit || Stay)
   > 이렇게 두가지 액션을 제외하고는 플레이어가 할 수 있는 액션이 없다. <br> 
   > 블랙잭을 판단하거나 카드 값을 계산하는 것은 플레이어의 책임이 아니다. <br>
   > 그렇다면 딜러는 어떻게 해야 할까? 딜러는 배팅을 하지도 않고, 카드를 받을지 결정하지도 않는다. <br>
   > 딜러는 일종의 컴퓨터다. 정해진 규칙에 따라서만 행동하며, 여기에 본인의 의지를 담지 않는다. <br>
   > 딜러를 Player와 성격이 같다고 할 수 있는지 같은 클래스를 상속하는 개념으로 생각할 수 있는 고민이 된다.

4. Cards : 객체 Map<Card, Player> 일급 컬렉션

> Player를 작성하던 중 고민이 생겼다. 이번 과제의 요구사항 중 '3개 이상의 인스턴스 변수를 가진 클래스 금지' 조항이 있었다. <br>
> 그런데 Player를 작성할 때 `name`, `money`, `List<Card>` 이렇게 세 가지 인스턴스 변수가 필요한 상황이다. <br>
> 나는 이 문제를 `money`를 제외하는 방식으로 해결하려 했었다. (요구사항에서 `money`의 역할이 승패 개념이었기 때문 : `money`가 저장되지는 않았다.) <br>

그런데 2가지 의문이 들었다.
   1. 나중에 `money`를 저장하게 되면 결국 `Player`의 인스턴스 변수가 된다.
      > 우선 이건 오버 엔지니어링과 의미가 달랐다. 그렇게 생각한 이유는 다음과 같다 <br>
        현실 세계에서 블랙잭 게임판에 올라온 `money`의 의미는 플레이어의 돈이다. (그것이 현금이든 칩이든 기본적으로 플레이어의 것이다.) <br>
        이런 현실성을 고려했을 때 `money`를 단순히 한 게임에서만 관리하는 인스턴스가 되도록 하는 것은 실제와 맞지 않다고 느꼈다. <br>
        실제 도박이 아닌 경우 가상 머니를 가지고 (일종의 점수 개념) 하기도 하지만, 그런 경우 '배팅'이라는 행위가 마음에 걸렸다. <br>
        즉, `Player`가 `money`를 가지고 있고, 배팅 액션을 실행할 때 `money`를 차감하는 것이 더 직관적인 설계 방향이라고 판단했다. <br>
        객체지향적인 도메인 설계가 어려울 때는 현실성을 고려해서 짜는 것이 좋다고 생각한다. 클래스의 의미 자체가 현실 사물이나 개념을 프로그래밍화 한 것이니 말이다. <br>
   2. `Cards` 는 `Player`의 인스턴스 변수가 맞는가?
      > 이렇게 생각하고나니 오히려 `Card`가 `Player`에 귀속되는 것이 더 이상했다. <br> 
        물론 현실에서도 플레이어에게 주어진 카드는 플레이어의 카드라고 인식한다. <br>
        그런데 블랙잭에 참여하는 딜러를 포함한 모든 플레이어는 하나의 카드 덱을 가지고 게임을 한다 <br>
        그리고 딜러의 첫 번째 카드를 제외하고 플레이어가 받은 모든 카드는 공개되며, 필드에 올라간다. <br>
        이런 관점에서 본다면 `Map` 을 사용해서 각 `Card`마다 `player`를 연결 시키는 것이 더 적절하다 생각했다. <br>
        중복된 카드는 존재할 수 없으며, 현재 카드가 누구의 카드인지를 인식하는 것으로 생각한다면 그게 나아보였다. <br>
...
> `CardDeck`을 구현 한 후 `Cards`가 수행해야 할 일을 보다 명확하게 정해줘야 했다. <br>
> 나는 `Cards`의 역할을 게임에서 드로우 된 카드를 관리하는 것으로 지정했다. <br>
> 그렇게 `Map`을 통해 `Card` 와 `Player`를 연결 시키고, `Player`를 기준으로 `Player`를 값으로 가진 카드의 정보와 값을 리턴하는 메서드를 구현했다. <br>
> 다만 마음에 걸리는 부분은 `Cards`의 역할이 제대로 설정된 것이 맞는지에 대한 의문이다. <br>
> `getPlayerCardsSumValue()`, `getPlayerCards()` 두 메서드는 `List<Card>`를 리턴하지 않고, 각각 `String`과 `int`를 리턴하도록 했다. <br>
> List를 리턴할 경우 리턴된 List가 조작될 수 있기 때문이었다. <br>
> 이로 인해 `Cards`의 리턴 값을 노출하기만 되는 View 입장에서는 명확한 역할과 책임의 처리가 가능할 것 같았다. <br>
> 그러나 `Cards`가 너무 많은 책임을 가진 것은 아닌지. 특히 `getPlayerCards()` 의 경우 `Computer::computeCardValue()`를 사용하고 있는데 이게 괜찮은건지 의문이다. <br>
> 또한 List<Cards>의 리턴부가 없다면 `Computer::testCondition()` 역시 `Cards`에서 사용해야 할 것이고, `Computer` 가 public static 으로 관리되는 것이 맞는지도 의문이다. <br>
...
6. Dealer : 'Croupier' 라는 단어로도 사용되고 있으나 요구사항에서 '딜러' 라고 표현했기에 `Dealer` 로 네이밍
> `CardDeck` 클래스를 구현하고 나니 `Cards`의 역할이 애매했다. 딜러가 있는 상황에서 블랙잭 게임을 플레이 할 때 플레이어가 직접 카드는 드로우 하는 일이 있을까? <br>
> 딜러가 카드를 드로우 한 후 공개하기 때문에 나는 `Cards`는 플레이어가 가지고 있으면 안된다는 판단을 내렸다. <br>
> 우선 게임에서 공동으로 사용하는 카드를 `Player` 인스턴스 변수로 가지고 있으면 안되다는 점은 아직도 동일하다. <br>
> 그런데 나는 `Cards`가 필요한지를 고민하고 있다. <br>
> 나는 플레이어가 직접 카드를 드로우 하는 메서드를 만들지 않고, 딜러가 카드를 드로우 해서 플레이어에게 배정하는 것을 염두에 두고 있다. <br>
> 이 경우 딜러는 블랙잭 게임 그 자체가 될 것이다. <br>
> 결국 게임을 관리하는 클래스는 필요하다. 블랙잭이 해당 역할을 수행할 것이다. 블랙잭은 다음과 같은 액션을 수행한다.
> 1. 딜러를 포함, 플레이어의 List를 가진다.
> 2. `CardDeck`을 가진다.
> 3. 플레이어와 카드를 연결시킨다. (카드가 누구에게 배정되었는지)
> 
> 블랙잭의 역할은 컨트롤러인가? 유틸인가? <br>
> 생각을 할 수록 머리가 복잡해졌다. 그래서 우선 Cards를 먼저 리팩토링하기로 했다. Map 형태로 카드와 플레이어를 연결시키는 역할만 수행하도록 할 것이다. <br>